...
def request_read(document_ids: list[int]) -> list[int]:
    """Queue documents for reading; returns the ones actually added."""
...
async def read_one(document_id: int) -> None:
    ...
    try:
        summary = await read_document(document_id, progress)
