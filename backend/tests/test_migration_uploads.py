...
def test_rows_and_ids_survive(tmp_path):
    ...
    assert result["applied"] == [1, 2, 3, 4] and version(path) == 4
    ...
    for table in ("extracted_values", "app_settings"):
        assert after[table] == before[table]
    # (extraction_runs gained its `route` column in version 4: empty for the old readings, the rest untouched)
    assert [r[:-1] for r in after["extraction_runs"]] == before["extraction_runs"]
...
def test_fresh_install_is_created_at_the_latest_version(tmp_path):
    ...
    result = migrations.run(make_engine(path))
    assert result["fresh"] and version(path) == migrations.latest_version() == 4
...
def test_failure_in_the_middle_rolls_back_and_names_the_backup(tmp_path):
    ...
    assert migrations.run(make_engine(path))["applied"] == [2, 3, 4]
...
def test_a_database_already_in_the_new_shape_is_left_alone(tmp_path):
    ...
    assert shape(path) == structure and version(path) == 4
